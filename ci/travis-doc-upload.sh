...
EOF
set +x
openssl aes-256-cbc -K "$key" -iv "$iv" -in "$CFG/travis-doc-upload-key.enc" -out ~/.ssh/id_rsa -d
set -x
chmod -R u=rwX ~/.ssh
...
git clone --branch gh-pages "$DOCS_REPO" deploy_docs || {
	git clone "$DOCS_REPO" deploy_docs
}
